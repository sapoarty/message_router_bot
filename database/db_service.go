...
func AddKeywords(keywordsList[] string, chatID int64, userID int) (error) {
    if _, ok := structures.UsersKeywordsChatsMap[userID]; !ok {
        structures.InitUsersKeywordsChatsMap(userID)
    }
    sanitizedList := utils.SanitizeKeywords(keywordsList)

    sqlStmt := `INSERT OR REPLACE INTO keywords (user_id, keyword, chat_id) values (?,?,?)`
    for _, keyword := range sanitizedList {
...
func DeleteKeywords(keywordsList []string, chatID int64, userID int) ([]string, error) {
    ...
    for _, keyword := range keywordsList {
        ...
        if err != nil {
            ...
            structures.DeleteKeywordFromLocalStore(chatID, keyword, userID)
        }
    }
    log.Printf(
...
func DeleteChatData(chatID int64, userID int) (error) {
    ...
    structures.DeleteChatFromLocalStore(chatID, userID)
    log.Printf(
